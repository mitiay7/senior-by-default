...
### 4.1.0 Branch name verification (REQUIRED)

Before composing the commit, verify the worktree's branch matches `config.naming` template. Auto-named branches (`claude/<adj>-<noun>`, `agent/<random>`, etc.) MUST be renamed.
...
if [ "$ACTUAL" != "$EXPECTED" ]; then
  ...
  # If old branch was already pushed, delete it remotely:
  git -C "$WORKTREE_PATH" push origin --delete "$ACTUAL" 2>/dev/null || true
  # Record in metrics — this is a Phase 2 spec violation
  RENAMED_BRANCH=true
fi
```

If `RENAMED_BRANCH=true`, include in metrics entry: `"branch_rename": {"from": "$ACTUAL", "to": "$EXPECTED"}` — this signals Phase 2 worktree-creation didn't follow spec, useful for skill diagnostics.
...
The final announcement MUST include verifiable confirmation of every step that ran. Format:

```
Complete. Branch: {branch}. PR: {url or "—"}. CI: {green/red/skipped/n-a}. Auto-merge: {on/off}.
[+ if context_doc updated → "Context: {context_doc.path} §N[, §M] updated."]
[+ if ADR → "ADR-{NNNN} committed."]
[+ if config.metrics.log_path set → "Metrics: {COUNT} entries in {LOG_PATH}." (from §4.11 Step 4 verification)]
[+ if config.metrics.log_path set AND emission failed → "Metrics: APPEND FAILED — {reason}." DO NOT silently omit.]
[+ if config.metrics.log_path NOT set → omit metrics line entirely]
```

**The metrics line is mandatory whenever `config.metrics.log_path` is configured** — either positive (entries count) or negative (failure reason). Silent omission means §4.11 was skipped, which is a bug.
