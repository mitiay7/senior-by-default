...
**Telemetry-integrity release.** Built from a read of the 93 canonical entries the logs have accumulated since the v0.10.0 archive cut. Five of the six telemetry findings are defects in the *measurement* rather than in the pipeline being measured — a gate that fired on the median case, a rate that could print 200 %, a vocabulary that split one bucket into three, duplicate entries counted twice, and a feature shipped in v0.12.0 that produced nothing on its only opportunity to fire. The sixth is the largest real defect class the pipeline still ships into review. Shipping in the same release: the pre-push secret gate that scanned the wrong directory and called it a pass, and the release gate that could not certify a change developed the way this skill mandates.
...
### Changed — PR-size warn caps come from the run's complexity tier

The warn thresholds were flat 800 lines / 20 files for **every** tier, while the H bucket's own plan-time cap is 25 files / 1500 lines. The two gates contradicted each other by construction: an H plan approved at 1400 lines PASSed Phase 2.0 and was **guaranteed** to WARN at Phase 3.0 for being exactly the size it had been authorized to be. Measured: `pr_size` warned on **43 of 86 runs — 80 % of all H runs**, so amber carried no information; and `calibration_size`, which scores the Phase 2.5 self-review's size prediction *against this gate*, sat at **53 %** — a coin flip on a dimension v0.10.0 had deliberately de-confounded from `calibration_defect` (75 %) to make readable.

- New `pr-size-check --tier T|L|M|H` derives the warn caps from that tier's plan bucket (T 2f/50L · L 3/200 · M 8/600 · H 25/1500 — the same table `plan-size-check` gates the plan against). [`phase-3-review.md`](skills/do/references/phase-3-review.md) §3.0 passes the routed tier. WARN now states one interpretable fact: **the delivered diff exceeded the budget its own plan was approved against.**
- Precedence, per dimension: explicit `--warn-*` > `config.pr_size.*` > tier bucket > flat defaults. The verdict line names the source it used — `[warn caps: tier H]`, `[warn caps: config]`, `[warn caps: lines=cli files=config]`.
- **Block caps are unchanged and deliberately tier-independent** — an unreviewable diff is unreviewable whoever planned it. This is also what keeps [`do-pr-size-pretooluse.sh`](skills/do/hooks/do-pr-size-pretooluse.sh) honest without a tier: it only ever DENIES on BLOCK, so the two enforcement tiers can differ on advisory WARN *text* and never on an enforced verdict — the lea-docs#1399 invariant holds.
- Omitting `--tier` reproduces pre-0.13 behavior byte-for-byte.
- Expected effect on the same cohort: H warn 80 % → 57 %, M 22 % → **40 %** (M's flat 800 was *looser* than its own 600-line bucket — the gate gets stricter exactly where it was too permissive). The residual 57 % on H is not threshold noise; it is the finding that H plans systematically under-estimate, now readable instead of constant.

### Added — Phase 2.5 self-review must mutation-verify every new guard

Across 93 runs, **~25 % of all Phase 3 criteria failures were one class**: a new condition written, never covered, whole suite green. The reviewer's own words from telemetry: *"scope==crm guard was mutation-provably untested"*, *"second ON CONFLICT condition mutation-provably untested"*, *"emptying all three dialogs onSuccess left the entire 818-test admin suite green"*.

New step 3 in [`phase-2-implementation.md`](skills/do/references/phase-2-implementation.md) §Self-Review: enumerate every new/changed condition (`if`/`switch` arms, `WHERE`/`ON CONFLICT` predicates, middleware and route guards, permission/ownership/tenancy checks, retry and timeout branches, early returns, success callbacks), **invert it in place, re-run only the named test, and require it to go red**, then restore. A guard whose inversion can't be tested cheaply goes under `Deferred (code)` **with the reason** — that is a signal Phase 3 can act on; claiming `ready` over an unverified guard is what `calibration_defect` scores as a false positive. Reported on a new machine-readable `guard_coverage: N new, N mutation-verified, N deferred` line. Phase 3.7 gained a matching acceptance criterion: a guard visible in the diff but absent from the tally gets spot-checked by the reviewer, and a suite that stays green is a FAIL with criterion `guard-coverage`.

### Fixed — `metrics-report` block-rate was a ratio of two different units

`block%` was `total blocker records / auditor appearances`, so one audit filing three findings scored 300 %. The shipped report printed `silent-failure-hunter 200%` and an implausible `backend-architect 100%`. It is now computed per **iteration**: the denominator is the number of review cycles the agent took part in (auditor **or** blocker — an agent that blocked while missing from `.auditors` is still a participant, and including it is what makes the rate ≤100 % by construction), and the numerator is the cycles in which it filed at least one blocker. Raw finding counts survive as their own `findings` column. Corrected figures on the same data: security-auditor 43 %, code-reviewer 41 %, database-architect 47 %, backend-architect 67 %. The reader also now folds blockers written as bare strings, which previously scored as zero.

### Fixed — four telemetry-schema drifts `metrics-append` now catches

Each is a real production entry, and each was a silent corruption of an aggregate rather than a visible error:

- **`scope` is a controlled vocabulary** — `backend | frontend | fullstack | infra | docs`. 93 entries carried **eleven spellings of six concepts** (`backend` 27, `Backend` 24, `B` 3, `Frontend` 14, `frontend` 4, `F` 1, …), so every per-scope aggregate split one bucket into three. Case is folded and shorthands expanded; anything outside the list is **preserved with a stderr NOTE**, never rejected — a hard failure on a descriptive field would push the caller toward the §19a direct-write bypass, losing the whole entry to save one string.
- **Duplicate `(ref, started_at)` REJECTs** — i1183 is a byte-identical duplicate line, i1233 appears twice with different bodies; both were double-counted in every tier count, gate rate and calibration figure. Identity is the pair, not the ref, so a genuine re-run of one issue (which has its own start time) is unaffected. `--allow-duplicate-ref` covers two real runs starting inside the same second. **If the wrapper printed `OK`, the append succeeded — do not retry it.**
- **`specialist_audit` verdict beside `specialist_iterations: []` REJECTs** — the gate says N agents reviewed the diff, the array says none ran. It parsed, passed the schema gate, and contributed nothing to the specialist aggregate, which is how it survived 14 of 34 audited runs (i1540 recorded `auditors: 4, approvers: 4` in the gate details next to an empty array). Same class as the existing `outcome`/`blocked_reason` check. `skipped` remains the honest "none ran".
- **Bare-string `blockers[]` are lifted to `{agent: …}`** — i1233 wrote both cycles as string arrays, which killed every consumer doing `.blockers[].category` with `Cannot index string with string`. `category`/`summary` are left **absent, not stubbed**; inventing them would be the exact fabrication the anti-stub gate exists to catch. Mixed namespaced/bare agent names in one entry now emit a NOTE (the namespaced form is canonical; consumers fold on `split(":") | last`).

### Fixed — the v0.12.0 token hook had fired zero times

Between shipping on 2026-08-08 and this audit there was exactly **one** `/do` run, and its entry carried no `tokens` key: one opportunity, zero amends, no error anywhere. The hook was registered, executable and identical to the dev copy — it simply never matched. That run was conducted in Russian, and the self-scoping guard was an anchored grep for the English prose lines `Complete. Branch:` / `Models: orchestrator=`.

- **The spec is the fix**: [`phase-4-finalize.md`](skills/do/references/phase-4-finalize.md) now states that the §4.13 announce is a **protocol block, emitted verbatim in English regardless of session language**, because two hooks parse it. Prose around it stays in the user's language.
- **The hook is hardened, not made clever**: it additionally triggers on the machine-readable `Metrics:` line (a literal key, a count, a path — no translatable words) rather than enumerating translations. Scoping is unaffected; the real check was always structural (the announce must name a ref that is on the resolved log and still lacks `tokens`).
- **`DO_TOKENS_DEBUG=1`** prints the reason for every silent no-op. Any hook whose failure paths are all silent successes needs this from day one — shipping without it is what turned a one-line guard mismatch into a release-long blind spot.
- `do-metrics-stop-gate.sh` was deliberately **not** widened the same way: it is enforcement, not enrichment, and widening an enforcement trigger changes what gets blocked. Its header now says so.

**Verified:** two new suites — [`tests/pr-size-tier-warn.test.sh`](tests/pr-size-tier-warn.test.sh) (37 assertions; TEST 3 reads each tier's caps off `plan-size-check`'s own output and asserts `pr-size-check` uses exactly those as the WARN boundary, so editing one wrapper's table alone goes red) and [`tests/telemetry-integrity.test.sh`](tests/telemetry-integrity.test.sh) (30 assertions; one case per defect above, including the 3-findings-in-1-cycle shape that used to print 300 %). Existing suites unchanged and green: `tokens-stop-amend` 23/23, `pr-size-generated-paths` 24/24, `secret-scan-worktree-scope` 21/21, `branch-normalize-remote-guard` 17/17, and `hook-live-sim.sh` 40/40. `shellcheck` clean on the changed scripts.
...
Also fixed a documentation contradiction found in the same audit: [`phase-0-setup.md`](skills/do/references/phase-0-setup.md) said auto-init writes `version + _meta + issue_tracker + issue_locale + specialists`, omitting `metrics` — but `config-init` actually writes the metrics preset too (its own `_setup_notes` and the `Metrics config: INCLUDED in auto-init` announce agree). The summary line now lists `+ metrics` with the correct `--no-metrics` omission caveat.
