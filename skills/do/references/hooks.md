...
1. **Self-scopes** identically to `do-metrics-stop-gate.sh` — no-op unless the last assistant message carries the /do final-announce signature, no-op on a spec-quoting turn (unexpanded §4.13 placeholders), `stop_hook_active` loop-guard. Safe to register globally.
