...
|---|---|---|
| `task_started` | After Phase 1 issue creation (M/H) OR after Phase 0 Step 6 announce (Low) | "Started {ref_format}: {title} → {url}" |
...
| `migration_proposed` | When Phase 0 Step 5 detects new migration | "Migration {TS} proposed in {ref_format} ({migration_dir})" |
