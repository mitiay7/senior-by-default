...
**Auto-init config (only if Step 1 set `CONFIG_FOUND=0` AND `--no-config-init` not in `$ARGUMENTS`)** — **run this bash verbatim** (do not Write a `config.json` by hand, do not compose `$CONFIG_LINE` yourself — see [anti-patterns §19c](anti-patterns.md)). The full annotated shape of what gets written (and every field you might add later) is in [`config-authoring.md`](config-authoring.md); this block only invokes the `config-init` wrapper:
...
The generated file contains: `version + _meta + issue_tracker + issue_locale + specialists + metrics` (the `specialists` block is omitted when `--no-specialists` is passed or the verified preset came back empty; the `metrics` block is omitted when `--no-metrics`/`--metrics none` is passed — matching `config-init`'s own `_setup_notes` and the `Metrics config: INCLUDED in auto-init` announce above). The specialists preset references plugins from the two recommended marketplaces (`anthropics/claude-plugins-official` + `wshobson/agents` — see [README](../../../README.md#recommended-claude-code-plugins-for-phase-3-specialist-review)). `_meta._setup_notes` in the generated file lists the exact install commands, including per-missing-plugin restore commands when the preset was filtered.
