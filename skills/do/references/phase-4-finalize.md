...
- `Phase 4.1: SECRETS INCONCLUSIVE` (exit 4) → **nothing was scanned** and the push did NOT run (the `elif` is false, so the block takes the withheld branch). The range held zero commits, which in practice means the wrapper was pointed at the wrong directory — a checkout sitting on `origin/main` while the branch being pushed lives in another worktree (lea-docs#1463). This line is **not** gate evidence and must never land in `gates.secret_scan` as a pass. Fix: re-run 4.1.2 with `-C` pointing at the directory the push actually runs from — the verdict line's `[dir: …]` names what was scanned, compare it with `$WORKTREE_PATH`. If the task ends here: `gates.secret_scan = { "status": "fail", "details": { "reason": "<the INCONCLUSIVE line>" } }`.
...
>
> **`0 commits, 0 files` is not a pass, and used to print like one.** Until lea-docs#1463 an empty range exited 0 with `SECRETS PASS`, so a scan of the wrong checkout produced a verdict line that read exactly like a successful one — and this section told the model to file it as proof. Reproduced live on 2026-08-13: a commit containing a password assignment was pushed while the gate reported `range f1c451e..f1c451e clean (0 commits, 0 files)`. Now that state is exit 4 with its own text, and every verdict line carries `[dir: <path>]` so the wrong-directory case is visible at a glance instead of being indistinguishable from success.
...
**Computing `$TOKENS_IN` / `$TOKENS_OUT`** (optional): populate ONLY from harness-reported usage for this task (a usage/cost readout the harness itself surfaced). No harness figure available → leave both unset (the flags are omitted and the entry carries no `tokens` field — that absence is honest). **NEVER estimate, reconstruct from context size, or approximate chars/4** — a guessed number poisons downstream ROI math precisely because it is indistinguishable from a measured one ([`telemetry-internals.md`](telemetry-internals.md) §Why token accounting exists). **In practice no in-session actor can read its own usage** — the sanctioned source is the opt-in Stop hook `do-tokens-stop-amend.sh` ([`hooks.md`](hooks.md)), which after this turn ends sums the transcript's harness-recorded `message.usage` and back-fills the entry via `metrics-append --amend-tokens`. So on a normal run leave these flags unset here and let the hook fill them; pass them inline only when you genuinely hold a harness readout (e.g. a headless post-run figure).
