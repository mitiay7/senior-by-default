...
19a. **Writing to `$LOG_PATH` directly instead of calling `metrics-append`** — `echo "$JSON" >> "$LOG_PATH"`, `Write` tool against the log path, `python3 -c '...' >> "$LOG_PATH"`, manually `jq`-built JSON appended via `>>` — ALL forbidden. The only supported emission path is `~/.claude/skills/do/scripts/metrics-append` invoked by the §4.13 bash block. The wrapper enforces named-args, enum validation, JSON-payload validity, and atomic append.
    ...
    **Observed bypass shapes** (post-v0.6.0 audit, 7 cases): sub-agents fabricate JSON with completely different field names — `ts` / `timestamp` instead of `started_at`/`ended_at`, `insertions` / `deletions` / `lines_removed` instead of `lines_added` / `lines_deleted`, `event: task_completed` / `task: <ref>` instead of `ref:`, `verdict: APPROVE_WITH_NITS` / `outcomes` (plural) / `phase_4_status: ok` instead of `outcome:`. All 7 missed the same 5 canonical fields (`ref`, `started_at`, `ended_at`, `outcome`, `lines_deleted`) — pattern confirms direct Write/echo, not wrapper misuse. **No combination of field-name guessing will pass the wrapper's required-arg check** — sub-agents must call `metrics-append` with the documented `--<flag>` names, period.

19d. **Composing the Phase 2.0 plan-size check decision by hand instead of running `~/.claude/skills/do/scripts/plan-size-check`** — production-confirmed (post-v0.6.0 audit): 0 of 32 entries had `complexity_rebumped_from` set, despite 16 of 21 M-tasks shipping >600 lines or >8 files. Same fabrication pattern as caveman v1/v2: orchestrator reads the threshold values + case-statement inline in spec, decides "looks fine to me", composes no output, no signal that the check was skipped (because the rebump field is opt-out by design).

    v2 structural fix: detection moved to wrapper. The thresholds + bucket caps live ONLY in the wrapper, never in the spec. Spec invokes `PLAN_SIZE_LINE="$(~/.claude/skills/do/scripts/plan-size-check ...)"` and `case "$PLAN_SIZE_LINE" in` to decide. If the wrapper isn't run, `$PLAN_SIZE_LINE` is empty, the case-statement matches nothing — visible bug.

    **Anti-fabrication tell**: wrapper output includes the actual computed cap values for the bucket (e.g. `caps: 8 files / 600 lines` for M). Spec contains no copy-pasteable form. If an orchestrator's announce includes a `Phase 2.0: ...` line but the cap values don't match the wrapper's internal table for that complexity — fabrication tell.

    **Diagnostic**: if your task ended up with files/lines clearly exceeding bucket caps AND `complexity_rebumped_from` is absent in the metrics entry — orchestrator skipped the wrapper. Same correction: re-run, paste wrapper output verbatim.
...
19e. **Fabricating canonical `gates` keys / hand-composing gate vocabulary** — same data-quality class as the `outcome`-enum drift (§ above / v0.6.0) and the same root cause as every §19 entry: free-form composition where a controlled vocabulary is required. The May-24 audit (225 entries) found **~110 distinct gate keys for ~19 real gates** — `test`/`tests`/`test_gate`/`go_test`, `ui`/`ui_gate`/`visual_verify`/`visual_smoke`, `i18n`/`i18n_gate`, `contract`/`contract_gate`, `dep_vuln`/`dep_vuln_go`/`dep_vuln_pnpm`, `type_check`/`type-check`/`lint_typecheck` — plus equally-drifty statuses (`skip`/`n/a`/`n-a`/`na`/`n_a`, scalar `pass`/`true`/`ok`/`clean`). Each synonym splits one gate's stats across buckets → the daily-report gate-failure-rate is noise.

    Structural fix (v0.7.0): the `metrics-append` wrapper owns the vocabulary. It renames known aliases to canonical, coerces statuses to `pass|warn|fail|block|skipped`, and **preserves** (never drops, never rejects) unknown task-specific keys while counting them. You pass whatever name is natural at capture; the wrapper canonicalizes. So the anti-pattern is NOT "used a non-canonical name" (the wrapper handles that) — it's **bypassing the wrapper and writing your own gate JSON**, OR composing a `Metrics:` announce that claims canonical keys the wrapper never emitted. Same correction as §19a: emit only via `metrics-append`; the OK-line tell (`gates=<C> renamed=<R> noncanon=<list>`) is the anti-fabrication signal — an announce that can't be backed by a real wrapper run is a tell.

    **Diagnostic**: if the metrics entry's `gates` object has keys outside the canonical set AND they aren't legitimately task-specific (they're obvious synonyms like `tests` or `i18n_gate`), the entry was written by direct `>>`/Write, not the wrapper — the wrapper would have renamed them. Same as the §19a field-name-drift diagnostic.

19f. **Composing the Phase 3.0 PR-size verdict by hand instead of running `~/.claude/skills/do/scripts/pr-size-check`** — production-confirmed (254-entry audit): **8 PRs added >2000 lines and EVERY one recorded `pr_size=warn`, never `block`**, despite `block_lines=2000` (i807 3587L, i1167 3669L, i1100 2067L, i1168 2053L, …). The §3.0 spec said "> block → BLOCK … do NOT proceed to merge", but the orchestrator read the threshold values inline, eyeballed the diff, and decided "warn is fine" — a 4000-line unreviewable PR shipped as `ready_for_review`. Identical fabrication-skip pattern to §19d (plan-size) and §19b (caveman).

    Structural fix (v0.7.0): the PASS/WARN/BLOCK decision moved into the `pr-size-check` wrapper. The spec passes the real `git diff` numbers; the wrapper decides. **BLOCK exits 3** (a hard halt), so it cannot degrade to advisory. Anti-fabrication tell: the wrapper line includes the `breached: [..]` dimension list and the `+N lines/+N files` overage, computed from the actual numbers — a hand-composed `Phase 3.0:` line that doesn't match the wrapper's exact form (or claims WARN when the numbers exceed block) is the tell. **Diagnostic**: a metrics entry with `gates.pr_size.status="warn"` AND `details.lines > config.block_lines` is a fabricated verdict — the wrapper would have emitted `block` and exited 3.
