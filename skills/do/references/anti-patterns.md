...
31d. **Shell-injecting `{title}` or `{labels}` into tracker command strings** — user-controlled `$ARGUMENTS` reaches `{title}`. A title like `x" --milestone 5 --label injected "y` smuggles extra CLI flags through naive shell substitution into `gh`/`glab`/etc. Tracker commands MUST execute with argv-safe semantics: pass user-controlled values via env vars (`gh issue create --title "$TITLE" ...`) or use the argv-array `commands` form for custom trackers. See [`trackers.md`](trackers.md) §Security. Legacy string-template `commands` are deprecated and MUST be `shlex.quote`'d before substitution as a fallback.
